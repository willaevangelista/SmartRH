...
export default function SobreNos() {
  ...
  return (
    <section className="py-12 px-4 bg-gray-50 min-h-screen">
      <h1 style={{ color: '#0061FF' }} className="text-3xl font-bold text-center mb-10">Conheça a Equipe Desenvolvedora</h1>
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-2 max-w-3xl mx-auto">
        {pessoas.map((pessoa, index) => (
          <div key={index} className="bg-white rounded-2xl shadow-lg p-6 text-center hover:scale-105 transition-transform duration-300">
            <h2 style={{ color: '#0061FF' }} className=" text-xl font-bold mb-2">{pessoa.nome}</h2>
            <img
              src={pessoa.foto}
              alt={pessoa.nome}
            ...
            <div className="flex justify-center gap-4">
              <a href={pessoa.linkedin} target="_blank" className="text-blue-600 hover:underline">
                <LinkedinLogo size={48} weight='bold' />
              </a>
              <a href={pessoa.github} target="_blank" className="text-blue-600 hover:underline">
                <GithubLogo size={48} weight='bold' />
              </a>
              {pessoa.linktree && (
                <a href={pessoa.linktree} target="_blank" className="text-blue-600 hover:underline">
